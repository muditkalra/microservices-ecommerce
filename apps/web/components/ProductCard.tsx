		...
		<div className="shadow-lg rounded-lg overflow-hidden">
			...
			<Link href={`/products/${product.id}`}>
				<div className="relative aspect-[2/3]">
					<Image src={(product.images as Record<string, string>)?.[productTypes.color] as string || ""} alt={product.name} fill className="object-cover hover:scale-105 transition-all duration-300" />
				</div>
			</Link>
				...
				<div className="flex items-center gap-4 text-xs">
					<div className="flex flex-col gap-1">
						<span className="text-gray-500">
							<select name="size" id="size" className="ring ring-gray-300 rounded-md p-2 py-1" onChange={e => handleProductType({ type: "size", value: e.target.value })}>
								{
